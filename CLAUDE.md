...
- `404.html` — Warm Editorial 404 page using `layout: home` (big serif "404.", error receipt, detour cards)
- `projects.html` — legacy projects page using `layout: layout`
- `css/`
  - `home.css` — **site design system** (Warm Editorial: Fraunces + Inter + JetBrains Mono, light/dark tokens, squiggle nav). Also styles `/posts`, post pages, and the 404 page.
  - `base.css`, `post.css` — legacy styles used by `projects.html`
...
See [DESIGN.md](DESIGN.md) for the full design system: tokens, typography scale, component inventory, layout rules, and guidance for building new pages. Read it before creating a new page or inventing a new component. Update it in the same commit whenever you add a reusable pattern or change a token.
...
- **Components**: squiggle nav underlines (SVG), dark-mode toggle button in the footer (persisted to `localStorage`), work list rows, side projects grid, posts section
...
- The shared site nav lives in `_includes/nav.html` and is used by `posts.html` and `_layouts/post.html` via `{% include nav.html current="<section>" %}`. Nav items: home (`/`), posts (`/posts/`). The homepage (`index.html`) has its own inline nav because the home item uses an in-page anchor (`#home`); posts uses a routed link (`/posts/`).
